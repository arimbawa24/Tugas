...
public class CekWifi extends Application{
    ...
    private void notification() {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            NotificationChannel connect = new NotificationChannel(
                    CONNECT, "Connect", NotificationManager.IMPORTANCE_HIGH);//importance high diprioritas tingkat tinggi
            connect.setDescription("Connect");

            NotificationChannel disconnect = new NotificationChannel(
                    DISCONNECT, "Disconnect", NotificationManager.IMPORTANCE_LOW);
